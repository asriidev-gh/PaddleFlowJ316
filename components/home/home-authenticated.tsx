...
export function HomeAuthenticated() {
  ...
  const liveQueueGames = useMemo(
    () => (liveQueueingEnabled ? games : []),
    [games, liveQueueingEnabled],
  );

  const activeGames = useMemo(
    () =>
      mergeHomeGameSummaries(
        liveQueueGames.filter((game) => game.status !== "ended"),
        quickGames
          .filter((game) => game.status !== "ended")
          .map(quickGameListCardToHomeSummary),
      ),
    [liveQueueGames, quickGames],
  );
  ...
  const gamesLoading =
    isGamesListLoading ||
    (isGamesListFetching && liveQueueGames.length === 0 && quickGames.length === 0);
    ...
    <main className="min-h-screen px-6 py-6 pb-[calc(4.75rem+env(safe-area-inset-bottom))] lg:px-10 lg:pb-6">
      <section className="mx-auto flex max-w-7xl flex-col gap-6">
        <HomeDashboard
          ...
          showLiveQueueSessions={liveQueueingEnabled}
        />
      </section>
      <HomeMobileNav />
    </main>
